...
<div class="mt-4 relative flex flex-col min-w-0 break-words w-full">
            ...
            $getQuickBoardName = "SELECT * FROM `posts_board` WHERE boardName = '".$row['boardName']."' AND `boardHidden`= 0";
...
$getPopularPost = "SELECT * FROM `posts` WHERE `postCreation` >= DATE_SUB(NOW(), INTERVAL 1 DAY) ORDER BY `posts`.`postView` DESC LIMIT 5";
