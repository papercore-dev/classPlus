...
if (isset($serviceName)){
    ...
    if ($checkPreviousVisit_Result->rowCount() > 0){
        while($row = $checkPreviousVisit_Result->fetch()){
            $updateServiceAnalytics = "UPDATE `account_serviceAnalytics` SET `visitCount` = `visitCount` + 1 WHERE `userID` = '".$_SESSION['userID']."' AND `signMethod` = '".$_SESSION['signMethod']."' AND `boardID` = '".$serviceName."'";
            $updateServiceAnalytics_Result = $db->query($updateServiceAnalytics);
        }
    } else {
        $insertServiceAnalytics = "INSERT INTO `account_serviceAnalytics` (`userID`, `signMethod`, `boardID`, `visitCount`) VALUES ('".$_SESSION['userID']."', '".$_SESSION['signMethod']."', '".$serviceName."', 1)";
