...
if ($db->query($postToDB)){
    echo "<script>window.location.replace = '/view.php?id=".$_GET["postID"]."&error=글을 공지에 업로드했어요.';</script>";
    die;
}
else{
    ...
    //post to database and redirect to newly created post
    if ($db->query($postToDB)){
        echo "<script>window.location.replace = '/view.php?id=".$_GET["postID"]."&error=글을 공지에서 내렸어요.';</script>";
        die;
    }
    else{
        echo "<script>window.location.replace = '/view.php?id=".$_GET["postID"]."&error=고정에 실패했어요.';</script>";
